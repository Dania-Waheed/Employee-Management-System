...
import { UserIcon, MenuIcon, XIcon, LayoutGridIcon, Calendar, FileTextIcon, DollarSignIcon, Settings, ChevronRightIcon, LogOutIcon } from 'lucide-react'
            ...
            <div className="flex-1 px-3 space-y-0.5 overflow-y-auto">
                {
                    navItems.map((item) => {
                        ...
                        return (
                            <Link key={item.href}
                                to={item.href}
                                className={`relative flex items-center gap-3 px-3 py-2.5 rounded-lg text-[13px] font-medium group transition-colors
    ${isActive ? "bg-indigo-500/10 text-indigo-300" : "text-slate-400 hover:text-slate-200 hover:bg-white/4"}`}
                            >
                                {
